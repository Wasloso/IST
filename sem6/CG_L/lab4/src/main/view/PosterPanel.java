...
public class PosterPanel extends JPanel {
    ...
    private SelectedVertex selectedVertex = SelectedVertex.NONE;
    ...
    private class MouseInputHandler extends MouseAdapter {
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            selectedVertex = SelectedVertex.NONE;
        }
    }
    ...
    private class MouseDragHandler extends MouseAdapter {
        ...
        public void mouseDragged(MouseEvent e) {
            if (selectedElement == null || lastMousePosition == null || selectedVertex == SelectedVertex.NONE)
                return;
            ...
            if (selectedVertex == SelectedVertex.CENTER) {
    ...
    private double clamp(double value) {
        ...
        if (abs < minScale) abs = minScale;
        if (abs > maxScale) abs = maxScale;

        return Math.signum(value) * abs;
      }
    
    private double angleBetweenPoints(double x1, double y1, double x2, double y2) {
        return Math.atan2(y2 - y1, x2 - x1);
    }
    private Point2D getAnchorPoint(SelectedVertex selected) {
    ...
    public PosterElement getElementAt(Point point) {
        for (int i = elements.size() - 1; i >= 0; i--) {
            ...
            if (el.contains(point) || el.getSelectedVertex(point) != SelectedVertex.NONE)
                return el;
        }
        return null;
    }
    ...
    public BufferedImage getImage(int width, int height, Boolean includeAllElements) {
        if (includeAllElements == null) includeAllElements = false;
