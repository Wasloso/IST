...
DROP TABLE IF EXISTS Ticket_paths CASCADE;
...
CREATE TABLE Ticket_lines (
                            ...
                            fk_line INT NOT NULL,
                            PRIMARY KEY (fk_ticket, fk_line),
                            FOREIGN KEY (fk_ticket) REFERENCES Tickets(id_ticket) ON DELETE CASCADE ON UPDATE CASCADE,
                            FOREIGN KEY (fk_line) REFERENCES Lines(id_line) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE Ticket_paths (
                            fk_ticket INT NOT NULL,
                            fk_path INT NOT NULL,
                            fk_begin_stop INT NOT NULL,
                            fk_end_stop INT NOT NULL,
                            PRIMARY KEY (fk_ticket, fk_path),
                            FOREIGN KEY (fk_ticket) REFERENCES Tickets(id_ticket) ON DELETE CASCADE ON UPDATE CASCADE,
                            FOREIGN KEY (fk_path) REFERENCES Paths(id_path) ON DELETE CASCADE ON UPDATE CASCADE,
                            FOREIGN KEY (fk_begin_stop) REFERENCES Stops(id_stop) ON DELETE CASCADE ON UPDATE CASCADE,
                            FOREIGN KEY (fk_end_stop) REFERENCES Stops(id_stop) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TYPE ticket_period_type AS ENUM ('General', 'Line', 'Path');

ALTER TABLE Ticket_Types
DROP COLUMN type;

DROP type ticket_discount_type;

ALTER TABLE Ticket_Types
ADD COLUMNN period_type ticket_period_type;

ALTER TABLE Ticket_Types 
ALTER COLUMN period_type SET NOT NULL;

UPDATE Ticket_Types
SET period_type = 'General';
