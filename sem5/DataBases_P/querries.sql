...
--po indeksowaniu
"Sort  (cost=14.20..14.60 rows=162 width=16) (actual time=0.296..0.305 rows=163 loops=1)"
"  Sort Key: last_technical_inspection"
"  Sort Method: quicksort  Memory: 30kB"
"  ->  Seq Scan on vehicles  (cost=0.00..8.25 rows=162 width=16) (actual time=0.026..0.219 rows=163 loops=1)"
"        Filter: (last_technical_inspection < (now() - '1 year'::interval))"
"        Rows Removed by Filter: 137"
"Planning Time: 0.603 ms" -- 3ms mniej
...
WHERE technical_issues.status != 'Resolved'
ORDER BY technical_issues.report_date ASC

"Sort  (cost=403.36..404.68 rows=528 width=175) (actual time=3.233..3.349 rows=576 loops=1)"
"  Sort Key: technical_issues.report_date"
"  Sort Method: quicksort  Memory: 135kB"
"  ->  Hash Join  (cost=9.75..379.48 rows=528 width=175) (actual time=0.141..2.946 rows=576 loops=1)"
"        Hash Cond: (technical_issues.fk_vehicle = vehicles.id_vehicle)"
"        ->  Seq Scan on technical_issues  (cost=0.00..368.32 rows=528 width=175) (actual time=0.029..2.598 rows=576 loops=1)"
"              Filter: (status <> 'Resolved'::technicalissuestatusenum)"
"              Rows Removed by Filter: 7203"
"        ->  Hash  (cost=6.00..6.00 rows=300 width=4) (actual time=0.100..0.101 rows=300 loops=1)"
"              Buckets: 1024  Batches: 1  Memory Usage: 19kB"
"              ->  Seq Scan on vehicles  (cost=0.00..6.00 rows=300 width=4) (actual time=0.013..0.052 rows=300 loops=1)"
"Planning Time: 0.416 ms"
"Execution Time: 3.418 ms"


-- po indeksowaniu
"Sort  (cost=327.27..328.61 rows=538 width=175) (actual time=1.213..1.271 rows=576 loops=1)"
"  Sort Key: technical_issues.report_date"
"  Sort Method: quicksort  Memory: 135kB"
"  ->  Hash Join  (cost=20.72..302.87 rows=538 width=175) (actual time=0.210..0.914 rows=576 loops=1)"
"        Hash Cond: (technical_issues.fk_vehicle = vehicles.id_vehicle)"
"        ->  Bitmap Heap Scan on technical_issues  (cost=10.97..291.68 rows=538 width=175) (actual time=0.067..0.452 rows=576 loops=1)"
"              Recheck Cond: (status <> 'Resolved'::technicalissuestatusenum)"
"              Heap Blocks: exact=81"
"              ->  Bitmap Index Scan on idx_unresolved_technical_issues  (cost=0.00..10.84 rows=538 width=0) (actual time=0.040..0.040 rows=576 loops=1)"
"        ->  Hash  (cost=6.00..6.00 rows=300 width=4) (actual time=0.126..0.127 rows=300 loops=1)"
"              Buckets: 1024  Batches: 1  Memory Usage: 19kB"
"              ->  Seq Scan on vehicles  (cost=0.00..6.00 rows=300 width=4) (actual time=0.026..0.066 rows=300 loops=1)"
...
-- ta tabela zbyt czesto sie zmienia zeby dac do niej indeks
...
"Sort  (cost=378.87..379.23 rows=144 width=64) (actual time=6.937..6.951 rows=145 loops=1)"
"  Sort Key: fines.deadline"
"  Sort Method: quicksort  Memory: 37kB"
"  ->  Nested Loop  (cost=89.96..373.71 rows=144 width=64) (actual time=0.811..6.831 rows=145 loops=1)"
"        ->  Hash Join  (cost=89.67..319.73 rows=144 width=17) (actual time=0.756..2.962 rows=145 loops=1)"
"              Hash Cond: (passengers.id_passenger = fines.fk_passenger)"
"              ->  Seq Scan on passengers  (cost=0.00..159.53 rows=11053 width=8) (actual time=0.018..0.896 rows=11053 loops=1)"
"              ->  Hash  (cost=87.88..87.88 rows=144 width=17) (actual time=0.702..0.703 rows=145 loops=1)"
"                    Buckets: 1024  Batches: 1  Memory Usage: 16kB"
"                    ->  Seq Scan on fines  (cost=0.00..87.88 rows=144 width=17) (actual time=0.015..0.637 rows=145 loops=1)"
"                          Filter: ((status <> 'Paid'::finestatusenum) AND (deadline < now()))"
"                          Rows Removed by Filter: 3105"
"        ->  Index Scan using app_users_pkey on app_users  (cost=0.29..0.37 rows=1 width=55) (actual time=0.026..0.026 rows=1 loops=145)"
"              Index Cond: (id_user = passengers.fk_user)"
"Planning Time: 0.989 ms"
"Execution Time: 7.005 ms"

-- po indeksowaniu

"Sort  (cost=333.82..334.18 rows=144 width=64) (actual time=2.806..2.815 rows=145 loops=1)"
"  Sort Key: fines.deadline"
"  Sort Method: quicksort  Memory: 37kB"
"  ->  Nested Loop  (cost=44.91..328.66 rows=144 width=64) (actual time=0.273..2.730 rows=145 loops=1)"
"        ->  Hash Join  (cost=44.63..274.68 rows=144 width=17) (actual time=0.259..2.081 rows=145 loops=1)"
"              Hash Cond: (passengers.id_passenger = fines.fk_passenger)"
"              ->  Seq Scan on passengers  (cost=0.00..159.53 rows=11053 width=8) (actual time=0.024..0.777 rows=11053 loops=1)"
"              ->  Hash  (cost=42.83..42.83 rows=144 width=17) (actual time=0.217..0.218 rows=145 loops=1)"
"                    Buckets: 1024  Batches: 1  Memory Usage: 16kB"
"                    ->  Bitmap Heap Scan on fines  (cost=9.31..42.83 rows=144 width=17) (actual time=0.061..0.180 rows=145 loops=1)"
"                          Recheck Cond: ((deadline < now()) AND (status <> 'Paid'::finestatusenum))"
"                          Heap Blocks: exact=31"
"                          ->  Bitmap Index Scan on idx_fines_status_deadline  (cost=0.00..9.27 rows=144 width=0) (actual time=0.040..0.040 rows=145 loops=1)"
"                                Index Cond: (deadline < now())"
"        ->  Index Scan using app_users_pkey on app_users  (cost=0.29..0.37 rows=1 width=55) (actual time=0.004..0.004 rows=1 loops=145)"
"              Index Cond: (id_user = passengers.fk_user)"
"Planning Time: 0.720 ms"
"Execution Time: 2.869 ms" -- prawie 5ms mniej
...
"Sort  (cost=26.62..26.62 rows=2 width=34) (actual time=0.502..0.507 rows=3 loops=1)"
"  Sort Key: drivers_licenses.expires_on"
"  Sort Method: quicksort  Memory: 25kB"
"  ->  Nested Loop  (cost=9.18..26.61 rows=2 width=34) (actual time=0.271..0.452 rows=3 loops=1)"
"        ->  Hash Join  (cost=8.89..24.00 rows=2 width=24) (actual time=0.228..0.371 rows=3 loops=1)"
"              Hash Cond: (drivers.fk_license = drivers_licenses.id_license)"
"              ->  Seq Scan on drivers  (cost=0.00..13.00 rows=800 width=12) (actual time=0.052..0.138 rows=800 loops=1)"
"              ->  Hash  (cost=8.87..8.87 rows=2 width=20) (actual time=0.063..0.064 rows=3 loops=1)"
"                    Buckets: 1024  Batches: 1  Memory Usage: 9kB"
"                    ->  Bitmap Heap Scan on drivers_licenses  (cost=4.30..8.87 rows=2 width=20) (actual time=0.045..0.047 rows=3 loops=1)"
"                          Recheck Cond: (expires_on <= (CURRENT_DATE + '1 year'::interval))"
"                          Heap Blocks: exact=1"
"                          ->  Bitmap Index Scan on idx_drivers_license_date  (cost=0.00..4.29 rows=2 width=0) (actual time=0.018..0.018 rows=3 loops=1)"
"                                Index Cond: (expires_on <= (CURRENT_DATE + '1 year'::interval))"
"        ->  Index Scan using app_users_pkey on app_users  (cost=0.29..1.30 rows=1 width=18) (actual time=0.022..0.023 rows=1 loops=3)"
"              Index Cond: (id_user = drivers.fk_user)"
"Planning Time: 0.813 ms" -- << prawie 4ms mniej
